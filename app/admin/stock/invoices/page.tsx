...
export default function InvoicesPage() {
  ...
  const handleDownloadDeliveryNotePdf = async (invoice: Invoice) => {
    ...
    const { generateDeliveryNotePdf, applyStampToPdf } = await import(
      "@/lib/stock-document-pdf"
    );
  ...
  const invoicePaymentSummaries = useMemo(() => {
    const map: Record<string, { paidAmount: number; balanceRemaining: number }> =
      {};
    for (const inv of invoices) {
      if (inv.paymentSummary) {
        map[inv._id] = {
          paidAmount: Number(inv.paymentSummary.paidAmount ?? 0),
          balanceRemaining: Number(
            inv.paymentSummary.balanceRemaining ?? inv.subTotal,
          ),
        };
      }
    }
  ...
  if (loading) return <PageLoadingSkeleton title="Loading invoices" rows={8} />;
  ...
  const exportInvoicePdf = async (
    ...
    const { generateInvoiceStyleSummaryPdf } = await import(
      "@/lib/stock-document-pdf"
    );
        ...
        <CardContent className="p-0">
          {sortedInvoices.length === 0 ? (
            <EmptyState
              ...
              description={
                invoices.length === 0
                  ? "Create your first invoice or convert an existing quotation to start billing."
                  : "Try adjusting your search or sorting options."
              }
              actionLabel={
                invoices.length === 0 ? "Create quotation" : undefined
              }
              actionHref={
                invoices.length === 0
                      ...
                      >
                        <td className="px-3 py-2 align-top">
                          <div className="min-w-0">
                            <Link
                              href={`/admin/stock/invoices/${invoice._id}`}
                              className="truncate font-medium text-foreground hover:underline block"
                  ...
                  <MobileCard key={invoice._id}>
                    ...
                    <div className="grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                      ...
                      <p className="text-right capitalize">
                        Dispatch {dispatchState.replaceAll("_", " ")}
                      </p>
                    </div>
            ...
            <StickyActionBar label="Invoice actions">
              <Button asChild size="sm" variant="outline" className="flex-1">
                <Link href="/admin/stock/quotations?action=new">New quote</Link>
              </Button>
              <Button
                size="sm"
                className="flex-1"
