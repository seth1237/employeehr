...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const upstreamUrl = new URL(`/api/stock/public/quotations/${encodeURIComponent(id)}/pdf`, getErpBaseUrl())
    ...
    if (!upstreamResponse.ok) {
      ...
      return new NextResponse(errorText || "Failed to fetch quotation PDF", {
    ...
    console.error("Quotation PDF proxy failed:", error)
    return NextResponse.json(
      { success: false, message: error?.message || "Failed to proxy quotation PDF" },
      { status: 502 },
    )
  }
}
